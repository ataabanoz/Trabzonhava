...
async function initNotifications(){if('serviceWorker'in navigator)try{await navigator.serviceWorker.register('./sw.js?v=6.7')}catch(e){} const s=$('#notifyState');if(s)s.textContent=!('Notification'in window)?'Bu tarayıcı bildirim API’sini desteklemiyor.':`Bildirim izni: ${Notification.permission}`}
...
$('#testNotify')?.addEventListener('click',async()=>{if(Notification.permission!=='granted')return;const reg=await navigator.serviceWorker.ready;reg.showNotification('⚡ TrabzonHava test bildirimi',{body:'Erken uyarı bildirim altyapısı çalışıyor.',icon:'./icon-192.png',data:{url:'./?v=6.7#alerts'}})});
...
// --- v6.7 MODEL BUTONLARI + DOLU UST ATMOSFER ---
const WX_MODELS=[
...
let selectedWXModel='ecmwf_ifs025',modelCache={};

function setupModelDistrict(){
 const s=$('#modelDistrict'); if(!s)return;
 if(!s.options.length)Object.keys(D).forEach(n=>s.add(new Option(n,n)));
 if(!s.value)s.value=selected;
}
function firstArray(h,base){
 if(!h)return [];
 if(Array.isArray(h[base]))return h[base];
 const k=Object.keys(h).find(k=>k===base||k.startsWith(base+'_'));
 return k&&Array.isArray(h[k])?h[k]:[];
}
...
async function fetchOneModel(model,district){
 const p=D[district]||D.Ortahisar;
 const q=new URLSearchParams({
  latitude:String(p[0]),longitude:String(p[1]),
  hourly:'temperature_2m,precipitation,pressure_msl,temperature_850hPa,temperature_500hPa,geopotential_height_500hPa',
  models:model.id,forecast_hours:'48',timezone:'Europe/Istanbul'
 });
 const d=await modelJSON('https://api.open-meteo.com/v1/forecast?'+q);
 const h=d.hourly||{};
 return {time:h.time||[],t2:firstArray(h,'temperature_2m'),rain:firstArray(h,'precipitation'),
  msl:firstArray(h,'pressure_msl'),t850:firstArray(h,'temperature_850hPa'),
  t500:firstArray(h,'temperature_500hPa'),z500:firstArray(h,'geopotential_height_500hPa')};
}
function setUpper(x,i){
 const val=(a,fmt)=>a?.[i]!=null&&Number.isFinite(Number(a[i]))?fmt(Number(a[i])):'Veri yok';
 $('#m850').textContent=val(x.t850,v=>v.toFixed(1)+' °C');
 $('#m500t').textContent=val(x.t500,v=>v.toFixed(1)+' °C');
 $('#m500g').textContent=val(x.z500,v=>Math.round(v).toLocaleString('tr-TR')+' m');
 $('#mMsl').textContent=val(x.msl,v=>Math.round(v)+' hPa');
}
function renderOneModel(model,x,district){
 const i=currentIndex(x.time);
 $('#modelDetailTitle').textContent=`${model.name} • ${district}`;
 setUpper(x,i);
 $('#modelHours').innerHTML=x.time.slice(i,i+24).map((t,j)=>{
  const k=i+j,tm=new Date(t).toLocaleTimeString('tr-TR',{hour:'2-digit',minute:'2-digit'});
  const temp=x.t2[k]!=null?Number(x.t2[k]).toFixed(1)+'°C':'—';
  const rain=x.rain[k]!=null?Number(x.rain[k]).toFixed(1)+' mm':'—';
  const u850=x.t850[k]!=null?Number(x.t850[k]).toFixed(1)+'°C':'—';
  return `<article><b>${tm}</b><span>🌡️ ${temp}</span><span>🌧️ ${rain}</span><span>850 hPa ${u850}</span></article>`;
 }).join('');
 document.querySelectorAll('[data-model]').forEach(b=>b.classList.toggle('active',b.dataset.model===model.id));
}
async function openModel(modelId){
 setupModelDistrict();
 const district=$('#modelDistrict')?.value||selected;
 const model=WX_MODELS.find(m=>m.id===modelId)||WX_MODELS[0];
 selectedWXModel=model.id;
 $('#modelStatus').className='status';
 $('#modelStatus').textContent=`${district} • ${model.name} verisi alınıyor…`;
 $('#modelDetailTitle').textContent=model.name+' • yükleniyor…';
 ['#m850','#m500t','#m500g','#mMsl'].forEach(id=>$(id).textContent='Yükleniyor…');
 $('#modelHours').innerHTML='';
 try{
  const key=district+'|'+model.id;
  const x=await fetchOneModel(model,district);
  if(!x.time.length||!x.t2.length)throw Error('boş veri');
  modelCache[key]=x; renderOneModel(model,x,district);
  $('#modelStatus').className='status ok';
  $('#modelStatus').textContent=`${district} • ${model.name} hazır • modele dokunarak değiştirebilirsin`;
 }catch(e){
  $('#modelStatus').className='status err';
  $('#modelStatus').textContent=`${district} • ${model.name} verisi alınamadı`;
  $('#modelDetailTitle').textContent=model.name+' • veri alınamadı';
  ['#m850','#m500t','#m500g','#mMsl'].forEach(id=>$(id).textContent='Veri alınamadı');
 }
}
async function loadModels(){
 setupModelDistrict();
 modelCache={};
 await openModel(selectedWXModel);
 // Mutabakat is intentionally separate; it must never block selected-model display.
 const district=$('#modelDistrict')?.value||selected;
 $('#modelConsensus').innerHTML='<b>Model mutabakatı</b><p>Diğer modelleri yukarıdaki düğmelerden tek tek açabilirsin. Seçilen modelin 24 saat ve üst atmosfer değerleri anında bu sayfada gösterilir.</p>';
}
setupModelDistrict();
$('#modelRefresh').onclick=()=>openModel(selectedWXModel);
$('#modelDistrict').onchange=()=>openModel(selectedWXModel);
document.querySelectorAll('[data-model]').forEach(b=>{
 b.type='button';
 b.onclick=()=>openModel(b.dataset.model);
});
$('#modelCards').onclick=e=>{const b=e.target.closest('[data-pickmodel]');if(b)openModel(b.dataset.pickmodel)};
